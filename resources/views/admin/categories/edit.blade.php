...
@section('title', 'Edit Category | Deal24hours')
@section('content')
<style>
    .error {
        color: #dc3545;
    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center gap-1">
                ...
                <a href="{{ route('admin.categories.index') }}" class="badge border border-secondary text-secondary px-2 py-1 fs-13">
                    ← Back to List
                </a>
            </div>
            <div class="card-body">
                    ...
                    <div class="row gy-3">
                        ...
                        <div class="col-md-12">
                            ...
                            <input type="text" name="slug" id="slug" class="form-control" 
                                   placeholder="Enter custom slug or leave blank" value="{{ old('slug', $category->slug) }}" />
                        </div>
                        ...
                        <div class="col-md-12">
                            ...
                            <select name="parent_id" id="parent_id" class="form-select">
                                ...
                                @foreach($mainCategories as $mainCategory)
                                    @if($mainCategory->id != $category->id) {{-- Prevent selecting self as parent --}}
...
$(document).ready(function () {
    ...
    function validateForm() {
        ...
        if (!name || name.length < 3) {
            ...
            $('#name').after('<span class="error error-message">Category name must be at least 3 characters long</span>');
            toastr.error('Category name must be at least 3 characters long');
            isValid = false;
        } else if (name.length > 255) {
            $('#name').addClass('is-invalid');
            $('#name').after('<span class="error error-message">Category name cannot exceed 255 characters</span>');
        ...
        if (parentId && isNaN(parentId)) {
            ...
            $('#parent_id').after('<span class="error error-message">Please select a valid parent category</span>');
        ...
        if (parentId == "{{ $category->id }}") {
            $('#parent_id').addClass('is-invalid');
            $('#parent_id').after('<span class="error error-message">A category cannot be its own parent</span>');
            toastr.error('A category cannot be its own parent');
            isValid = false;
        }
    ...
    $('#categoryForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            complete: function() {
                $('button[type="submit"]').prop('disabled', false).html('Update Category');
            }
        });
    });
});
