...
@section('title', 'Edit Vendor | Deal24hours')
@section('content')
<style>
    .error {
        color: #dc3545;
                    ...
                    <div class="row gy-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="email" name="email" id="email" class="form-control" 
                                   placeholder="Enter email" value="{{ old('email', $vendor->email) }}" required>
                        </div>

                        <div class="col-md-6">
                            <label for="phone" class="form-label">Phone <span class="text-danger">*</span></label>
                            <input type="text" name="phone" id="phone" class="form-control" 
                                   placeholder="Enter phone" value="{{ old('phone', $vendor->phone) }}" required>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="status" id="status" class="form-select" required>
                                ...
                                <option value="2" {{ old('status', $vendor->status) == '2' ? 'selected' : '' }}>Inactive</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="store_name" id="store_name" class="form-control" 
                                   placeholder="Enter store name" value="{{ old('store_name', $vendor->store_name) }}">
                        </div>

                        <div class="col-md-6">
                            <label for="gst_no" class="form-label">GST Number</label>
                            <input type="text" name="gst_no" id="gst_no" class="form-control" 
                                   placeholder="Enter GST number" value="{{ old('gst_no', $vendor->gst_no) }}">
                        </div>

                        <div class="col-md-6">
                            <label for="country" class="form-label">Country</label>
                            <input type="text" name="country" id="country" class="form-control" 
                                   placeholder="Enter country" value="{{ old('country', $vendor->country) }}">
                        </div>

                        <div class="col-md-6">
                            <label for="state" class="form-label">State</label>
                            <input type="text" name="state" id="state" class="form-control" 
                                   placeholder="Enter state" value="{{ old('state', $vendor->state) }}">
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="pincode" id="pincode" class="form-control" 
                                   placeholder="Enter pincode" value="{{ old('pincode', $vendor->pincode) }}">
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="file" id="gst_doc" name="gst_doc" class="form-control" accept=".pdf,.jpg,.jpeg,.png">
                            
                            @if(!empty($vendor->gst_doc))
                                <div class="file-actions" id="gstDocActions">
                                    <a href="{{ asset('/'.$vendor->gst_doc) }}" target="_blank" class="btn btn-sm btn-outline-primary">
                                        <i class="bi bi-eye-fill"></i> View Document
                                    </a>
                                    <button type="button" class="btn btn-sm btn-outline-danger" onclick="confirmDeleteGstDoc()">
                                ...
                                @if(pathinfo($vendor->gst_doc, PATHINFO_EXTENSION) !== 'pdf')
                                    <div id="gstDocPreview">
                                        <img src="{{ asset('/'.$vendor->gst_doc) }}" alt="GST Document" class="img-thumbnail document-preview mt-2">
                                    </div>
                                @endif
                                
                                <input type="hidden" name="existing_gst_doc" value="{{ $vendor->gst_doc }}">
                            @endif
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="file" id="store_logo" name="store_logo" class="form-control" accept=".jpg,.jpeg,.png">
                            ...
                            @if(!empty($vendor->store_logo))
                                ...
                                <div id="logoPreview">
                                    <img src="{{ asset('/'.$vendor->store_logo) }}" alt="Store Logo" class="img-thumbnail mt-2" style="max-height: 150px;">
                                </div>
                                <input type="hidden" name="existing_store_logo" value="{{ $vendor->store_logo }}">
                            @endif
                            <div id="newLogoPreview" class="mt-2 d-none">
                                <img id="logoPreviewImage" src="#" alt="Logo Preview" class="img-thumbnail" style="max-height: 150px; display: none;">
                            </div>
                        </div>
...
$(document).ready(function () {
    ...
    function validateForm() {
        ...
        if (!name || name.length < 3) {
            ...
            $('#name').after('<span class="error error-message">Name must be at least 3 characters long</span>');
        ...
        if (!email) {
            ...
            $('#email').addClass('is-invalid');
            $('#email').after('<span class="error error-message">Please enter a valid email address</span>');
    ...
    $('#vendorForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            complete: function() {
                $('button[type="submit"]').prop('disabled', false).html('<i class="bi bi-save"></i> Update Vendor');
            }
        });
    });
...
function confirmDeleteGstDoc() {
    if (confirm('Are you sure you want to remove the GST document?')) {
        ...
        input.value = '1';
        document.getElementById('vendorForm').appendChild(input);
