...
class VendorController extends Controller
{
    ...
    /**
     * Fetches vendors data and renders a Blade partial for AJAX pagination.
     * This method will return the HTML content (table body + pagination links).
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Contracts\View\View
     */
    public function renderVendorsTable123(Request $request)
    {
        ...
        $vendorsQuery = User::query()
            ->where('role', 'vendor')
            ->with('vendorProfile') // Eager load vendorProfile to avoid N+1 queries
        ...
        ->when($request->status !== null && $request->status !== '', function($q) use ($request) {
            // Convert status to integer as it might come as string from AJAX
            // 0 for inactive, 1 for active based on your HTML select options
        ...
        // })
         ->when($request->gst_no, function($q, $gstNo) {
            $q->whereHas('vendorProfile', function($q2) use ($gstNo) {
                // --- Changed from LIKE to = for exact match ---
                $q2->where('gst_no', $gstNo);
                // --- End Change ---
            });
        })
        ->orderBy('name', 'asc'); // Ensure a consistent order for pagination
        ...
        // This view is expected to contain the <tbody> and <tfoot> elements
    ...
    public function startExport(Request $request)
    {
        ...
        // Store current filter parameters for the export
        $filters = $request->only(['name', 'email', 'status', 'gst_no', 'phone']);

        Cache::put("export_progress_{$exportId}", [
            'progress' => 0,
            'message' => 'Initializing export...',
            'status' => 'started',
            'filters' => $filters // Store filters for later use in download
        ], 3600); // Cache for 1 hour

        // In a real application, you would dispatch a job here:
        // ExportVendorsJob::dispatch($exportId, $filters);
    ...
    public function getProgress(Request $request)
    {
        ...
        if (!$progress) {
            return response()->json(['status' => 'failed', 'message' => 'Export not found or expired.'], 404);
        }

        // Simulate progress increment for demonstration
        if ($progress['progress'] < 100) {
            $progress['progress'] += 10; // Increment progress by 10%
            $progress['message'] = "Processing data: {$progress['progress']}%";
            if ($progress['progress'] >= 100) {
                $progress['progress'] = 100;
                $progress['message'] = 'Export completed!';
                $progress['status'] = 'completed';
                // Add the download URL when completed
                $progress['download_url'] = route('admin.vendors.export.download', ['export_id' => $exportId]);
            }
            Cache::put("export_progress_{$exportId}", $progress, 3600); // Update cache
        }
    ...
    public function download(Request $request): StreamedResponse
    {
        ...
        if (!$progress || !isset($progress['filters'])) {
            abort(404, 'Export data not found or expired.');
        }

        $filters = $progress['filters'];

        $filename = "vendors_export_" . Carbon::now()->format('Ymd_His') . ".csv";
        $headers = [
        ...
        // Use StreamedResponse to handle large files efficiently
        $callback = function() use ($filters) { // Pass filters to the callback
            try {
                $file = fopen('php://output', 'w');

                // Add CSV headers
                fputcsv($file, [
                    'ID', 'Name', 'Email', 'Phone', 'Store Name', 'GST No', 'Pincode', 'Address',
                    'Status', 'Is Verified', 'Approved Products', 'Pending Products', 'Created At'
                ]);

                // Build the query using the stored filters
                $vendorsQuery = User::query()
                    ->where('role', 'vendor')
                    ->with('vendorProfile')
                    ->withCount([
                        'products as approved_products_count' => function ($q) { $q->where('status', 'approved'); },
                        'products as pending_products_count' => function ($q) { $q->where('status', 'pending'); }
                    ]);

                // Apply filters
                $vendorsQuery->when($filters['name'] ?? null, function($q, $name) {
                    $q->where('name', 'like', "%{$name}%");
                })
                ->when($filters['email'] ?? null, function($q, $email) {
                    $q->where('email', 'like', "%{$email}%");
                })
                ->when($filters['status'] ?? null !== '' && $filters['status'] !== null, function($q) use ($filters) {
                ...
                // --- IMPORTANT CHANGE: Use chunkById to process data in chunks ---
                // Fetch data in chunks of 1000 records at a time
                $vendorsQuery->chunkById(1000, function ($vendors) use ($file) {
                    foreach ($vendors as $vendor) {
                        ...
                        fputcsv($file, [
                            ...
                            $vendor->email,
                            $vendor->phone,
                            $profile->store_name ?? 'N/A',
                            $profile->gst_no ?? 'N/A',
                            $profile->pincode ?? 'N/A',
                            Str::limit($profile->address ?? 'N/A', 150),
                            ...
                            Carbon::parse($vendor->created_at)->format('d M Y H:i:s'),
                        ]);
                    }
                });
